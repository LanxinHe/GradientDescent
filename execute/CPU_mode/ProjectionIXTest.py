...
from model.CPU_model import ProjectionIX
import torch
...
if __name__ == '__main__':
    ...
    #                                                                                                         RNN_HIDDEN_SIZE,
    #                                                                                                         STEP_SIZE)
    detnet = ProjectionIX.DetModel(TX, RNN_HIDDEN_SIZE, PROJECT_TIMES, RES_FACTOR)
    ...
    with torch.no_grad():
        ...
        for ebn0 in TEST_EBN0:
            ...
            with torch.no_grad():
                ...
                BER += [ber]
    # --------------------------------------- Save Model & Data --------------------------------------------------------
    PATH = '../../pretrained_projectionIX/tx%i/rx%i/rate%i/EBN0_Train%i/iterations%i/project_times%i/batch_size%i/rnn_hidden_size%i/step_size%.5f/res_factor%.2f/' % (TX, RX, RATE,
                                                                                                            EBN0_TRAIN,
                                                                                                            ITERATIONS,
                                                                                                            PROJECT_TIMES,
                                                                                                            BATCH_SIZE,
