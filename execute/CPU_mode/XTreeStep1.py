...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCHS):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            loss = tree_step1(outputs, label, RATE)
            loss.backward()
            optim_det.step()
    ...
    with torch.no_grad():
        ...
        for ebn0 in TEST_EBN0:
            ...
            with torch.no_grad():
                ...
                BER += [ber]
    # --------------------------------------- Save Model & Data --------------------------------------------------------
    PATH = '../../pretrained_XTree/tx%i/rx%i/rate%i/EBN0_Train%i/iterations%i/project_times%i/batch_size%i/rnn_hidden_size%i/step_size%.5f' % (TX, RX, RATE,
                                                                                                            EBN0_TRAIN,
                                                                                                            ITERATIONS,
                                                                                                            PROJECT_TIMES,
                                                                                                            BATCH_SIZE,
